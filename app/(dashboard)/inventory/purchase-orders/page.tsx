...
import {
  ShoppingCart, Search, Filter, RefreshCw, Plus, Edit, Trash2, MoreHorizontal, Building2, Calendar, Package,
  CheckCircle2, Clock, ChevronRight, LayoutGrid, List, SortAsc, User, Loader2
} from 'lucide-react';
...
export default function PurchaseOrdersPage() {
  ...
  const handleBulkDelete = async () => {
    if (!confirm(`Are you sure you want to delete ${selectedRows.size} purchase orders?`)) return;
    try {
      const ids = Array.from(selectedRows).join(',');
      const res = await fetch(`/api/inventory/purchase-orders/bulk?ids=${ids}`, { method: 'DELETE' });
    ...
    <div className="w-full h-full flex flex-col bg-slate-50/50">
      ...
      <div className="px-6 py-4 flex items-center justify-between bg-white border-b border-slate-200 shrink-0">
        ...
        <div className="flex items-center gap-2">
          <Button className="h-8 px-4 shadow-sm font-bold text-xs rounded-md" onClick={() => router.push('/inventory/purchase-orders/new')}>
            {tCommon('create', { entity: t('single') })}
          </Button>
          <MassActionsMenu entity="Purchase Orders" selectedCount={selectedRows.size} onAction={(action) => {
            import('@/components/tables/massActionsHandlers').then(mod => {
              const { handleMassAction } = mod;
              handleMassAction(action, 'Purchase Orders', selectedRows, orders as any[]);
            });
        ...
        <div className="flex-1 flex flex-col min-w-0 bg-white">
          ...
          <div className="h-12 px-4 border-b border-slate-100 flex items-center justify-between shrink-0">
            <div className="flex items-center gap-4 w-full max-w-xl">
              <div className="relative flex-1">
                ...
                <Input
                  placeholder="Find a purchase order..."
            ...
            <table className="w-full border-collapse">
              <thead className="bg-slate-50/80 sticky top-0 z-10 border-b border-slate-200">
                <tr>
                  ...
                  <th className="px-6 py-3 text-left text-[11px] font-bold text-slate-500 uppercase tracking-wider">Purchase Details</th>
                  <th className="px-6 py-3 text-left text-[11px] font-bold text-slate-500 uppercase tracking-wider">Status</th>
                  <th className="px-6 py-3 text-right text-[11px] font-bold text-slate-500 uppercase tracking-wider">Grand Total</th>
                  <th className="px-6 py-3 text-left text-[11px] font-bold text-slate-500 uppercase tracking-wider">Business Info</th>
                  <th className="px-6 py-3 text-left text-[11px] font-bold text-slate-500 uppercase tracking-wider">Ownership</th>
                ...
                {filteredOrders.length === 0 ? (
                  <tr>
                    <td colSpan={7} className="h-64 text-center">
                      <div className="flex flex-col items-center justify-center p-8">
                        ...
                        <p className="text-[14px] font-bold text-slate-900">No Procurement Records Found</p>
                      </div>
                    </td>
                  </tr>
                ) : (
                  filteredOrders.map(order => (
                    ...
                    >
                      ...
                      <td className="px-6 py-4">
                        <Badge variant="outline" className={`text-[10px] font-bold tracking-tight uppercase border px-2 py-0.5 rounded-md shadow-none ${STATUS_STYLE[order.status] || 'bg-slate-50 text-slate-700'}`}>
                          {tCommon(`statuses.${order.status.toLowerCase()}`, { defaultValue: order.status })}
                        </Badge>
                      </td>
                      <td className="px-6 py-4 text-right font-black text-slate-700 text-[13px]">
                        ${Number(order.grandTotal).toLocaleString(undefined, { minimumFractionDigits: 2 })}
                      </td>
                      <td className="px-6 py-4">
                        <div className="flex flex-col gap-0.5">
                          <span className="text-[13px] font-bold text-slate-700">{order.vendor?.name || '---'}</span>
                          <span className="text-[11px] text-slate-400 font-medium">{order.contact ? `${order.contact.firstName} ${order.contact.lastName}` : '---'}</span>
                        </div>
                      </td>
                      <td className="px-6 py-4">
                        <div className="flex items-center gap-2">
                      ...
                      <td className="px-6 py-4 text-right">
                        <DropdownMenu>
                          ...
                          <DropdownMenuContent align="end" className="w-48 rounded-xl shadow-xl border-slate-100 p-1">
                            <DropdownMenuItem className="rounded-lg font-bold text-xs py-2" onClick={() => router.push(`/inventory/purchase-orders/${order.id}`)}>
                              View Logistics
                            </DropdownMenuItem>
          ...
          <div className="h-10 px-4 bg-slate-50/50 border-t border-slate-100 flex items-center justify-between shrink-0">
            <div className="flex items-center gap-4 text-[11px] font-bold text-slate-400 uppercase tracking-tight">
              <span>{Math.min(filteredOrders.length, limit)} of {total} Records</span>
